...
class DigestAuth(PostResponseAuth):
    ...
    async def __call__(self, response_obj, req_obj):
        ...
        if ha2 is None:
            ha2 = bytes(md5(bytes_method + b':' + bytes_path).hexdigest(),
                        self.encoding)
        ...
        try:
            ...
            response = md5(b':'.join(
                (ha1, auth_dict['nonce'], ha2))).hexdigest()

        response_items = ['username="{}"'.format(str(usrname, self.encoding)),
                          'realm="{}"'.format(str(auth_dict['realm'],
                                                  self.encoding)),
                          'nonce="{}"'.format(str(auth_dict['nonce'],
                                                  self.encoding)),
                          'uri="{}"'.format(req_obj.path),
                          'response="{}"'.format(response),
                          'opaque="{}"'.format(str(auth_dict['opaque'],
                                                   self.encoding))]
        if qop is not None:
